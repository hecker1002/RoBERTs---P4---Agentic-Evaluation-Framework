...
# ==============================================================================
# Use Streamlit secrets for the API key for secure deployment
...
''' For Better Cateogiration in respective domain'''
DOMAIN_PROTOTYPES = {
     "QA": [
...
# Your original fine-tuning input string generator
def bert_input_string(idx: int, data: pd.DataFrame) -> str:
    ...
    # New and Previously Unused Features
    domain = row['domain']
...
# --- FIXED: Moved this function definition before main() ---
def generate_final_report(question, response_text, bert_features, bert_prediction, llm_judgment):
    ...
    st.metric("BERT Predicted Score (Instruction-Following Score)", value=f"{bert_score:.4f}", help=f"The confidence score from the fine-tuned BERT model, with the predicted label: {bert_label}")
    
    # Analyze and reason about the BERT score based on features
    st.markdown("### Analysis of BERT Features")
    st.write("The BERT model's prediction is based on the following key features from the response:")
    ...
    if bert_features.get('contradiction_detected', 0) == 1:
    ...
    if high_impact_features:
        ...
        st.write("No significant negative features were detected. The response is generally aligned with the prompt.")
    ...
    if llm_judgment:
        ...
        st.markdown(f"---")
        st.markdown(f"**LLM Judge's Overall Score: {overall_score}/10**")
        st.write(f"Overall Justification: {overall_justification}")
    else:
        st.error("❌ The LLM judge failed to provide a judgment. Please check the logs.")
...
def main():
    ...
    """)
    st.sidebar.markdown("---")
    st.sidebar.info("This app relies on an external API (OpenRouter) and a pre-trained BERT model. Make sure you have your API key configured in a `secrets.toml` file.")
    ...
    if st.button("Run Evaluation", key="evaluate_button", use_container_width=True):
        if not question or not response_text:
            ...
            with st.spinner("Running evaluation..."):
                try:
                    # Using a placeholder model for demonstration. Replace this with your specific model.
                ...
                try:
                    ...
                    # FIX: Add truncation=True to handle inputs that are too long
